'''
File description:
    This program is a multiport PnP program desgned to read multiple serial ports on the fly.
...
Overview:
- each thread checks a serial port, error causes the thread to sleep and try again, not terminate
- different ports may have different protocols/intervals
  different ports may have different maintenance procedures
